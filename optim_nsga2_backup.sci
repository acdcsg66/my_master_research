...
function [pop_opt, fobj_pop_opt, pop_init, fobj_pop_init] = optim_nsga2(ga_f, pop_size, nb_generation, p_mut, p_cross, Log, param)
    ...
    for It=1:nb_generation
        ...
        record1 = Indiv1; //debug
        record2 = Indiv2; //debug
        //
        // Crossover
        //
        for j=1:nb_couples
        ...
        for j=1:nb_couples
            for r1=1:rooms //x軸に同じ座標がないかチェック
                for r2=r1:rooms
                    ...
                    while end_flag == 0
                        end_flag = 1;
                        if (int8(Indiv1(j)(r1)) == int8(Indiv1(j)(r2)) & r1~=r2) & (int8(Indiv1(j)(r1+rooms)) == int8(Indiv1(j)(r2+rooms)) & r1~=r2) |..
                        (int8(Indiv2(j)(r1)) == int8(Indiv2(j)(r2)) & r1~=r2) & (int8(Indiv2(j)(r1+rooms)) == int8(Indiv2(j)(r2+rooms)) & r1~=r2)..
                        //x軸y軸が同じ座標の個体ペアがあればやり直し
                            end_flag = 0;
                            Indiv1 = record1;
                            Indiv2 = record2;
                            //
        ...
        //
        for i=1:length(Indiv1)
            if ToCompute_I1(i) then FObj_Indiv1(i, :) = _ga_f(Indiv1(i)); end
            if ToCompute_I2(i) then FObj_Indiv2(i, :) = _ga_f(Indiv2(i)); end
        end
